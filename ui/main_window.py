# ----- Not Touch The Code It Not Use Pregard! -----
...
APP="Bord Profecional"
VER="0.3.21.72"
CRE="RockyDev, Inc"
REL="2026-02-27"
BUI="21.72000"
UPDATE="https://www.github.com/RockyDevFT/updates.json"
LICENCE="xxxxxxx-xxxxxxx-xxxxxxx-xxxxxxx {Activated}"
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(cfg["name"]+cfg["version"])
    ...
    def build_menu(self):
        ...
        file.addAction("📄Abrir Proyecto",self.open_project)
        file.addAction("📜Guardar Proyecto",self.save_project)
        file.addAction("🗂️Guardar como",self.save_as)
        file.addAction("💼Exportar PDF",self.export_pdf)
    ...
    def open_file(self):
        ...
        path,_=QFileDialog.getOpenFileName(
            self,"Abrir","","Texto (*.txt);;Todos (*.*);;Markdown (*.md);;Git (*.git)"
        )
        if not path:return
    ...
    def save_as(self):
        ...
        path,_=QFileDialog.getSaveFileName(
            self,"Guardar","","Texto (*.txt);;Todos (*.*);;Markdown (*.md);;Git (*.git)"
        )
        if not path:return
    ...
    def close_tab(self,i):
        ...
        if ed.toPlainText().strip():
            r=QMessageBox.question(self,"Guardar","¿Guardar cambios?")
